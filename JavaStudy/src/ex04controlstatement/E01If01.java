...
//		}
		...
		// if문의 조건식은 아래와 같이 boolean값(논리형)을 반환해야 한다. 
		// 비교식으로 처리.
		if (num%2==0) {
		...
		// (조건식 = boolean값) => 경우에 따라 무조건 실행되는 조건문이 필요한 경우 사용.
		if(true) {
		...
		/*
		 if(조건식);
		 => 세미콜론은 문장의 끝을 나타내므로 조건에 상관없이 중괄호에 있는 문장이 실행된다. 사용 주의.
		 */
		// false로 실행되면 안되지만, ;으로 인해 중괄호 내용이 실행됨.
		if (num%2 != 0); {
		...
		/*
		 ...
		 단, 실행문이 2개 이상이라면 첫 번째 문장만 if문에 포함되므로 반드시 중괄호를 기술해야 한다.
		 */
		if (num%2 == 0) 
			System.out.println("\n" + num + "은 짝수이다.");
		// false. 실행X
		...
		/*
		 ...
		 - 문자 0과 9는 아스키코드 48, 57에 해당하므로 구간 내에 있는지 확인하면 된다.
		 */
		...
		/*
		 아스키코드를 모를 때 구현방법.
		 - 문자 '0'과 '9'는 메모리에 저장 시 아스키코드로 변환되어 저장되므로
		   아래와 같은 조건으로도 동일한 결과를 확인할 수 있다.
		 */
		...
		if (asciiCode<'0' || asciiCode>'9') {
			System.out.println("입력한 문자는 문자입니다.[1]");
		}
		else {
			System.out.println("입력한 문자는 문자가 아닙니다.[1]");
		}
