...
//		return "" + data;
	 ...
	 * 인스턴스가 가진 멤버변수의 값에 대한 비교를 통해 동일 여부를 판단한다. equals()
	 * 메서드의 매개변수 타입은 모든 인스턴스를 대상으로 하므로 Object형으로 정의되어 있다.
	 */
	@Override
	public boolean equals(Object obj) {
		/*
		 * 매개변수로 전달된 인스턴스를 Object로 받으면 업캐스팅이 되므로 다운캐스팅 이후에
		 * 비교를 해야한다. 이때 전달된 인스턴스가 MyClass형이 아니라면 비교의 대상이
		 * 될 수 없으므로 상속관계 확인을 위해 instanceOf 연산자를 사용한다.
		 */
		if (obj instanceof MyClass) {
			/*
			 * 매개변수가 MyClass 타입이 맞다면 비교를 위해 다운캐스팅한다.
			 * 그렇지 않으면 부모타입으로는 자식 멤버에 접근할 수 없으므로 멤버변수에 대한
			 * 비교가 불가능하다.
			 */
			...
			/*
			 * 매개변수로 전달된 인스턴스가 MyClass타입이 아니라면 비교의 대상이
...
public class E04Object3Equals {
	...
	public static void main(String[] args) {
		...
		/*
		 * print를 통해 출력하면 자동으로 toString이 호출되므로 아래의 출력결과는 둘다
		 * 동일하다.
		 */
