...
public class Ex06HashMapMain {
	...
	public static void main(String[] args) {
		...
		/*
		 * 객체저장 : 저장 시 기존에 저장된 동일한 key값이 존재하면 저장된 인스턴스가 반환된다.
		 * 만약 처음이라면 null을 반환한다.
		 */
...
//		map.put("age", number);
		...
		/*
		 * 중복저장 : 기존에 입력된 name이라는 Key가 있기 때문에 '홍길동'이 반환된다. 그리고
		 * 기존의 값은 '최길동'으로 수정된다.
		 */
		System.out.println("name이라는 키값으로 저장된 이전의 값:"
					+ map.put("name", "최길동")); //덮어쓰기
		//기존의 값을 덮어쓰기 하므로 그대로 4개가 출력된다.
		...
		/*
		 * 	3.이터레이터를 통한 출력
		 * : 위와 동일하게 keySet()으로 얻어온 후 반복출력한다.
		 */
		...
		/*
		 * Value만 얻어와야 할때
		 * : values() 메서드를 통해 값만 얻어올 수 있다. 컬렉션의 형태로 얻어온 후 반복해서
		 * 출력한다.
		 */
		...
		/*
		 * 객체의 존재유무 확인
		 * : Map은 key와 value가 있으므로 메서드도 2개로 나눠져 있다.
		 */
