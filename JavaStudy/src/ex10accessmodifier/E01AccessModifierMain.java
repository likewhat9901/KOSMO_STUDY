...
//		one.privateMethod(); //에러발생
		/*
		 private으로 선언된 멤버에 외부클래스에서 접근하려면 default 혹은
		 public으로 선언되어 외부접근이 가능한 메서드를 통해 간접적으로 접근해야 한다.
		 클래스 내부에서는 접근지정자의 영향을 받지 않으므로 해당 메서드를 통해 접근이 가능해진다.
		 */
		one.defaultMethod();
		one.publicMethod();
		...
		/*
		 ...
		 또한 default 클래스는 동일 패키지 내에서 접근할 수 있다.
		 */
