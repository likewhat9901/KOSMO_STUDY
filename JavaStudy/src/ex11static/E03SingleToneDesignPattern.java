...
class NoSingleTone{
	 ...
	 * 클래스 외부 혹은 다른 패키지에서는 호출할 수 없기 때문에 new를 통해 인스턴스 생성
	 * 자체가 불가능하기 때문이다.
	 */
	public NoSingleTone() {}
}
...
class SingleTone {
	...
	int shareVar;
	/*
	 * 정의한 클래스와 동일한 타입으로 선언된 정적 멤버변수로 JVM에 의해 프로그램이 시작될때
	 * 미리 Method 영역에 로드되어 사용할 준비를 마치게 된다.
	 */
	private static SingleTone single = new SingleTone();
	
	/*
	 * 생성자를 private으로 선언하면 클래스 외부에서는 호출할 수 없으므로 new를 통해
	 * 인스턴스 생성을 할 수 없다.
	 */
	private SingleTone() {}
...
public class E03SingleToneDesignPattern {
	public static void main(String[] args) {
		...
		/*
		 * 일반적인 방식의 인스턴스 생성이므로, new로 생성할 때마다 새로운 참조값을 할당받게 된다.
		 */
