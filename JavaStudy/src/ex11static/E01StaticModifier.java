 ...
 * - main() 메서드가 실행되기 전에 초기화되므로 main() 메서드 실행시 별ㄹ도의 선언없이
 * 	변수를 즉시 사용할 수 있다.
 * 
 * 접근방법
 * - 클래스 내부
...
class MyStatic {
	 ...
	 * 접근해야한다. 즉, 일반적인 멤버변수는 인스턴스를 생성하는 시점에 메모리에 로드된다.
	 ...
	 * 정적 멤버만 접근이 가능하다.(※ main 메서드에서는 static으로 선언된 메서드만
	 * 호출할 수 있었던 것을 기억하자)
	 */
...
public class E01StaticModifier {
	...
	public static void main(String[] args) {
		...
		/*
		 * static(정적) 변수 접근방법: 클래스명을 통해 직접 접근한다. 인스턴스를 
		 * 생성하지 않고 바로 사용할 수 있다.
		 */
		...
		/*
		 * 정적변수도 일반적인 멤버변수처럼 참조변수를 통해 접근할 수 있지만, Java에서는
		 * 권장하지 않는다.
		 */
		...
		/*
		 * 인스턴스형 변수는 각각의 인스턴스 내부에 존재하므로, 아래 2개의 값은 서로
		 * 다르게 출력된다.
		 */
		ms1.instanceVar = 100;
		 ...
		 * 변수로 접근하든 동일한 메모리가 된다. 따라서 값을 공유하게 되므로 동일한
		 * 결과를 출력한다.
		 */
		ms1.staticVar = 800;
		ms2.staticVar = 900;
		...
		/*
		 * 결과를 보듯 입력시에는 각각의 참조변수를 사용했지만 결국 하나의 정적변수에 값을 
		 * 입력한 결과가 되므로, 정적변수에 접근 시에는 참조변수를 사용하지 말고, 클래스명을
		 * 사용하는 것이 좋다.
		...
		/*
		 * 정적 멤버 메서드에 접근방법은 정적변수와 동일하다. 참조변수를 통해서는 인스턴스형,
		 * 정적형 둘다 접근할 수 있다.
		 */
