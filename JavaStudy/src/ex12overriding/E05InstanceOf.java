...
/*
 * instanceof 연산자
 * : 인스턴스 변수가 어떤 타입의 변수인지를 판단하는 연산자로 형변환(즉, 상속관계가 있으면)이 가능하면
 * true를 반환한다.
...
public class E05InstanceOf {
	/*
	 * - 메서드 정의 시, 부모타입의 매개변수로 설정하면 자식 타입의 인수를 한꺼번에 받을 수 있다.
	 * - Box 타입으로 인수를 받으므로 자동형변환(업캐스팅)되어 자식 클래스의 멤버메서드를 
	 * 호출할 수 없게 된다.
	 * - 이때 자식의 멤버메서드를 호출하기 위해 강제형변환(다운캐스팅)이 필요하다.
	 * - instanceof 연산자를 통해 자식타입으로 형변환이 가능하지 판단한 후 true를 반환하면
	 * 다운캐스팅을 진행한다.
	 */
	...
	/*
	 * 만약 부모타입의 매개변수로 자식타입의 인수를 받을 수 없다면 아래와 같이 각 타입에 따른
	 * 모든 메서드를 정의해야 한다.
	 */
//	static void wrapBox(Box b) {}
