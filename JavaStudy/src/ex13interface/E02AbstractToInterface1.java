...
/*
 * DTO(Data Transfer Object) : 데이터를 저장하고 전송하기 위한 용도의 객체를 의미한다.
 * VO(Value Object)로 표현하기도 한다. 프로그램에서 이름과 주민번호를 저장하는 용도로 사용된다.
 */
class PersonalInfoDTO {
	...
	/*
	 * getter/setter : DTO객체에서 private으로 선언된 멤버변수의 값을 설정하거나 반환할때 사용.
	 */
...
class PersonalNumberStorageExt extends PersonalNumberStorageAbs {
	...
	/*
	 * 상속을 통해 추상메서드를 포함하게 되므로 반드시 아래와 같이 오버라이딩해야 한다. 이는 필수사항이며
	 * 지키지 않는다면 에러가 발생한다.
	 */
	@Override
	...
	public String searchPersonalInfo(String juminNum) {
		...
		for (int i = 0; i < numOfPerInfo; i++) {
			...
			if (juminNum.compareTo(personalArr[i].getJuminNum())==0) {
				...
				/*
				 * DTO의 멤버변수는 private으로 선언되었으므로 외부에서는 접근이 불가능하다.
				 * 따라서 getter를 통해 출력해야 한다.
				 */
			}
		}
