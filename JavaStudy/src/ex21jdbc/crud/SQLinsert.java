...
public class SQLinsert extends MyConnection{
	 ...
	 * 쿼리문을 실행해야 하므로 인스턴스가 생성된 후 변수를 초기화해야한다.
	 */
	...
	public void dbExecute() {
		try {
			/*
			 * Statement 인터페이스
			 * : 정적 쿼리문을 실행할 때 사용한다. 정적 쿼리문이란 인파라미터가
			 * 없는 완성된 쿼리문을 말한다. 수정은 개발자가 직접 진행해야 한다.
			 */
			stmt = con.createStatement();
			//쿼리문 작성
			query = "INSERT INTO member VALUES "
				+ " ('test2', '222', '테스터2', sysdate)";
			 ...
			 * : 인파라미터가 있는 동적쿼리문을 실행할 때 사용한다.
			 * 인파라미터는 ?로 표시하고, 인스턴스 생성 이후 setXX()
			 * 메서드를 통해 설정한다. 인덱스는 1부터 시작한다.
			 */
