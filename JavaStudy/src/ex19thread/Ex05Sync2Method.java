...
//	}
	...
	/*
	 * 동기화 방법2
	 * : 동기화 블럭을 사용한다. 동기화가ㅏ 필요한 코드에만 동기화하는 방법이다. 해당프로그램에서는
	 * 실행코드가 1줄밖에 없으므로 동기화 방법에 대한 성능차이가 없다. 하지만 실행코드가 많은 경우
	 * 메서드 전체를 동기화 하는 것보다 필요한 코드만 동기화하는 '동기화블럭'이 훨씬 더 좋은 성능을
	 * 보이게 된다.
	 */
...
class MultiAdderThread2 extends Thread {
	...
	/*
	 * 쓰레드를 통해 반복적으로 공유객체인 sumInst와 addNum()을 호출한다. 이 경우
	 * 동기화처리가 되지않아 동시에 접근되는 경우가 발생하게 되어 잘못된 결과가 출력된다.
	 */
	@Override
	public void run() {
...
public class Ex05Sync2Method {
	...
	public static void main(String[] args) {
		...
		/*
		 * 하나의 인스턴스를 2개의 쓰레드가 공유해서 사용하는 형태로 실행한다. 각각의 쓰레드가
		 * 5000번씩 반복해서 호출한다.
		 */
