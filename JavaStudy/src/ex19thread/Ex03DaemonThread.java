 ...
 * : 하나의 클래스가 작업을 완료해야 다음 클래스의 작업을 진행할 수 있다. 즉, 동시에 작업을
 * 진행할 수 없다.
 */
class NormalClass
{
 ...
 * : 메인쓰레드와 Working 쓰레드(개발자가 정의한 쓰레드)를 일컫는 것으로 메인쓰레드가 종료되어도
 * 계속 실행되며, Dead상태가 되어야만 실행이 종료되는 쓰레드를 말한다. 해당 쓰레드는
 * main()메서드가 종료된 이후에도 계속 실행되는 것을 볼 수 있다.
 */
class NormalThread extends Thread
{
	...
	public NormalThread() {}
	/*
	 * Thread 클래스의 생성자 호출시 전달하는 인수를 통해 이 쓰레드의 이름을 지정할 수 있다.
	...
	/*
	 * 쓰레드 동작 중 비정상적인 종료가 될때 InterruptedException이 발생할 수 있으므로,
	 * 예외처리를 해야한다. 여기서는 try~catch로 직접 처리하지 않고, throws해서 호출한
	 * 위치로 예외던지기를 하고 있다.
	 */
 ...
 * 메인메서드도 하나의 독립쓰레드이므로, 다른 독립쓰레드가 남아있다면 종료될 때까지는 프로그램이
...
public class Ex03DaemonThread {
	...
	public static void main(String[] args) {
		...
		/*
		 * 쓰레드로 구현하지 않은 일반적인 클래스의 작업으로 순차적으로 실행되어 1번 인스턴스가
		 * 작업을 마쳐야만 2번째가 실행된다.
		 */
		...
		/*
		 * 첫번째 독립스레드 생성 및 시작. 우선순위를 별도로 부여하지 않으므로 5로 지정된다.
