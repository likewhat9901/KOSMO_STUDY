...
class Account {
	/*
	 * 멤버변수가 상속관계에 있다 하더라도 private로 선언되면 클래스 내부에서만 접근할 수
	 * 있다. 즉 상속받은 하위 클래스에서는 직접 접근이 불가능하다.
	 */
	private int money;
	
	/*
	 * 이와같이 개발자가 생성자를 직접 정의하면 디폴트 생성자는 추가되지 않는다. 생성자를
	 * 정의하지 않은 경우에만 자동으로 추가된다.
	 */
...
class SavingAccount extends Account {
	 ...
	 * 현재 부모클래스에는 매개변수가 하나인 생성자가 유일하므로 만약 아래와 같이 호출하지
	 * 않으면 에러가 발생한다.
	 */
	public SavingAccount(int initVal) {
		/*
		 * 이 문장을 삭제하면 에러가 발생하게 된다. 삭제하는 경우 super()라는 문장,
	...
	public void showAccountMoney() {
		 ...
		 * 아래와 같이 getter메서드로 반환받아 출력해야 한다.
		 */
...
//		sa.money = 100000; //에러발생
		 ...
		 * money에 접근할 수 없다. 현재의 위치가 외부클래스이기 때문이다. 즉 private
		 * 멤버는 무조건 클래스 내부에서만 접근을 허용한다.
		 */
//		account.money = 1000; //클래스 외부에서 접근했기 때문에 에러.
