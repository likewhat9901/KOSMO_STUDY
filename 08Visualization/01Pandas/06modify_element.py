...
''' 0행 전체를 선택한 후 하나의 요소를 지정하게 되므로 이와 같은 형태로는 사용하지 않기를 권고 '''
# df.iloc[0][3] = 77 # FutureWarning 경고 발생 -> 값 변경은 됨.
...
# df.loc['서준']['체육'] = 66 # FutureWarning 경고 발생
df.loc['서준', '체육'] = 66
print(df)
...
''' 기존에 지정된 '이름'은 삭제되고, 체육으로 행의 index로 대체된다. '''
print(new_df1)
...
# 컬럼에서는 '국어'만 남게됨
print(new_df2)
# 인덱스가 2개이므로 요소를 지정할때도 아래와 같이 작성해야 한다.
