...
'''
데이터프레임(DataFrame)
    : 1차원 배열 형태인 시리즈가 여러개 모여있는 형태로 2차원배열과 동일하다.
...
'''
인덱스와 컬러럼명을 지정해서 데이터프레임을 생성.
인덱스는 행(Row)가 되고, 컬럼은 열(Column)로 지정된다.
'''
...
'''
df.drop('Student1', inplace=True) # 행이 Default값
print('삭제후1\n', df)

print('\n')
# Gender라는 행이 없으므로 오류가 발생된다.
# df.drop('Gender') # 열의 이름이므로 axis=1 로 지정해야 한다.

# 열(컬럼)에 대한 삭제이므로 축 옵션을 1로 지정해야 한다.
# 첫번째 실행에는 inplace 옵션이 없으므로 원본 객체에 적용되지 않는다.
df.drop('Gender', axis=1) # inplace 옵션의 Default는 False.
# 두번째 실행에서 원본 객체에 적용된다.
df.drop('Gender', axis=1, inplace=True) # 이거 주석처리하면 위에 코드는 반영되지 않은 것을 알 수 있음.
print('삭제후2\n', df)