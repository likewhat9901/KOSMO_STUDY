...
/*
 * 스프링컨테이너가 시작될때 빈이 생성되어야 하므로 기본패키지 하위에 정의한 후
...
public class WebSecurityConfig {
	 ...
	 * hasRole() : hasAnyRole()과 비슷하지만 한 가지의 권한을 획득해야 
	 * 	접근할 수 있다.
	 */
	@Bean
	...
	/*
	 * 로그인 후 획득할 수 있는 권한에 대해 설정한다. USER, ADMIN 권한을 획득하기
	 * 위한 아이디/비번을 메모리에 저장한다. DB연동을 위해서는 별도의 커스터마이징이
	 * 필요하다.
	 */
	@Bean
	public UserDetailsService users() {
		...
		/*
		 * 서버 실행될때마다 서로 다르게 나옴.
		 * 패스워드1번째 : {bcrypt}$2a$10$Vd0sRYVZGi212fEkdwVDFOVQdwnnjLLyqykjUpPAgaQABQPtoZHPK
		 * 패스워드2번째 : {bcrypt}$2a$10$rruzJE7uj5Vc.hGCyW9Qvu.miuz3YlytPyIe6PNDsO8d4mr7dOkvm
		 */
	...
	// Spring Security는 평문 비밀번호 저장을 허용하지 않음 → 반드시 필요
