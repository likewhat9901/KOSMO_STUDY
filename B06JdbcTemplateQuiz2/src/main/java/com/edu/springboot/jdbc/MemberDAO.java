...
public class MemberDAO implements IMemberService {
	 ...
	 * 설정하지 않는다. build.gradle에 의존성 추가가 되어있으면 스프링 컨테이너가
	 * 자동으로 빈을 생성해준다.
	 */
	@Autowired
	...
	//회원목록
	/* 
	 * 부모 인터페이스(IMemberService)에서 추상메서드를 수정하면 
	 * 자식인 DAO클래스에서도 함께 변경해야 한다.
	 */
	@Override
	public List<MemberDTO> select(MemberDTO memberDTO) {
		//회원레코드를 가입일을 내림차순 정렬해서 인출하는 쿼리문
	...
	public int insert(MemberDTO memberDTO) {
		 ...
		 * update() 메서드를 실행한다. 실행 후 적용된 행의 개수가 int형으로 반환된다.
		 */
		int result = jdbcTemplate.update(new PreparedStatementCreator() {
			/*
			 * PreparedStatementCreator 인터페이스로 익명클래스를 생성한 후
			 * 오버라이딩 된 메서드 내에서 인파라미터가 있는 insert쿼리문을 실행한다.
			 * 결과는 정수형으로 반한된다.
			 */
			@Override
	...
	public MemberDTO selectOne(MemberDTO memberDTO) {
		...
		try {
			memberDTO = jdbcTemplate.queryForObject(sql, 
					/*
					 * BeanPropertyRowMapper<MemberDTO>(MemberDTO.class)
					 * SQL 결과를 MemberDTO에 자동으로 매핑해주는 도구
					 * DB의 컬럼명과 DTO의 필드명이 이름이 같을 경우 자동으로 연결해줌
					 */
					new BeanPropertyRowMapper<MemberDTO>(MemberDTO.class), 
					new Object[] { memberDTO.getId() });
			/*
			 * queryForObject 메서드는 반드시 하나의 결과가 있어야 한다. 만약
			 * 잘못된 조건으로 인해 결과가 인출되지 않는다면 예외가 발생하게 된다.
			 * 따라서 try~catch로 감싸서 예외처리해주는 것이 좋다.
			 */
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public int update(MemberDTO memberDTO) {
		...
		/*
		 * PreparedStatementSetter 인터페이스의 익명클래스를 이용해서 인라미터를
		 * 설정한 후 쿼리문을 실행한다.
		 */
