...
<body>
	...
	<!-- <c:set> 태그는 기본적으로 page 영역에 저장돼.
❗ 하지만 scope 속성을 지정하면 다른 영역(request, session, application)에도 저장 가능 -->
	<c:set var="lists" value="<%= lists %>" />
	<!-- 
	저장한 변수를 통해 확장 for문을 실행한다. items에 지정한 컬렉션에
	...
	<%
	/* Java 코드를 사용하면 출력할 인스턴스 명시 후 getter를 직접 호출해야 한다. */
	for( Person per : lists) {
		out.print("이름: "+ per.getName() + ", 나이: "
			+ per.getAge() + "<br/>");
	}
	%>
	...
	<h4>Java 코드를 통한 출력</h4>
	<%
	// Map은 항상 Key값을 먼저 얻어와야 한다.
	Set<String> keys = maps.keySet();
	// 얻어온 Key값을 통해 개수만큼 반복한다.
	for(String key : keys) {
		// key를 통해 Value를 얻어온다.
