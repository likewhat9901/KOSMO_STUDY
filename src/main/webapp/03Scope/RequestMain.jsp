...
request 영역
	...
	즉, request 영역에 저장된 정보는 현재페이지와 포워드된 페이지까지 공유된다.
*/
// 리퀘스트 영역에 2개의 속성을 저장
...
<body>
	...
	<ul>
		...
		<!-- 멤버변수는 private으로 정보은닉이 되어 있으므로 getter메서드를 이용해서
		값을 출력해야 한다. -->
	...
	포워드 : 현재 페이지로 들어온 요청을 다음 페이지로 보내는 기능을 말한다.
		...
		즉 포워드란 현재 페이지로 들어온 요청을 다른 페이지에서도 고융할 수 있도록
		전달하는 것으로, 주소줄에는 최초 요청을 받은 페이지명이 나오고 웹브라우저에는
		요청이 전달된 페이지가 출력된다.
		request
		.getRequestDispatcher("RequestForward.jsp?paramHan=한글&paramEng=English")
		.forward(request, response);
	/*
	포워드 시 2개의 파라미터를 전달한다. 또한 request 영역에 저장된 속성도 함께 공유된다.
	*/
	%>
</body>
</html>