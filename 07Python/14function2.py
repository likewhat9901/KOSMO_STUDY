...
def sum(arg1, arg2):
  # 함수의 지역변수로 정의
  total = arg1 + arg2
  # 전역변수와 지역변수가 충돌되면 지역변수가 우선순위가 높다.
...
def person(name, age):
  ...
  myName(name)
  print(myAge(age))
# 외부함수를 호출하면 전달된 인수는 내부함수로 전달되어 결과 출력.
person('성유겸', 13)
...
# 매개변수명을 그대로 사용하여 값 전달. 따라서 순서 변경 가능
