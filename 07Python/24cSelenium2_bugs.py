...
""" 
셀레니움은 크롬 브라우저를 이용해서 크롤링 할 페이지를 띄운 후 데이터를 가져온다.
...
# 뷰티플숩을 임포트한 후 얻어온 정보를 Soup객체로 변환
...
for song in songs:
  try:
    # 노래제목
    title = song.select_one('th > p > a').text #CHARTrealtime > table > tbody > tr:nth-child(1) > th > p > a
    # 가수
    singer = song.select_one('td:nth-child(8) > p > a').text #CHARTrealtime > table > tbody > tr:nth-child(1) > td:nth-child(8) > p > a
    # 앨범
    album = song.select_one('td:nth-child(9) > a').text #CHARTrealtime > table > tbody > tr:nth-child(1) > td:nth-child(9) > a
...
pd_data.set_index('서비스', inplace=True)
# 데이터프레임의 상위 5개 행을 출력해서 확인
print(pd_data.head())
pd_data.to_excel('./saveFiles/bugs_chart.xlsx', index=True)
