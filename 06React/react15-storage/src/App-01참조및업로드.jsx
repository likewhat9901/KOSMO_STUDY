...
// 저장소에 대한 참조생성 및 파일업로드 함수 import
import { ref, uploadBytes } from "firebase/storage";

function App() {
  // firebase storage 연결 및 확인
  const storageRef = ref(storage); // 연결
  console.log('storageRef', storageRef);

  /* ref()를 호출할때 경로를 두번째 인수로 전달하여 경로 트리에서 하위위치를
  가리키는 참조객체를 만들 수 있다. */
  // 경로 : root/images
  const imagesRef1 = ref(storage, 'images');
  const imagesRef2 = ref(storage, 'images/myFile.jpg');
  /* parent 및 root 속성을 사용해서 한단계 상위로 이동하거나 최상위 경로를 참조할 수도 있다. */
  const imagesRef3 = imagesRef2.parent; // => /images
