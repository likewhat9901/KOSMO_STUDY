/* react-router-dom으로부터 import한 컴포넌트와 관련 Hook */
import { Routes, Route, Link, NavLink } from "react-router-dom";
import { Outlet, useLocation, useSearchParams } from "react-router-dom";

const TopNavi = () => {
  /* NavLink 컴포넌트는 a태그와 같이 하이퍼링크를 제공한다.
  단, a태그에 preventDefault()가 적용된 형태로 화면의 깜박임없이 페이지
  ...
  return (<>
    <nav>
      ...
      {/* a태그를 사용하는 경우 화면의 깜박임(새로고침?)이 있으므로 이벤트 객체를 통해
...
/* Outlet 컴포넌트
  : 웹사이트 제작시 공통으로 사용되는 레이아웃에서 특정 요청에 따른 내용만
  변경해야 할때 사용한다. */
const CommonLayout = () => {
  return (<>
    ...
    <article>
      ...
      <Outlet /> {/* 자식 Route 컴포넌트들이 여기 렌더링됨. */}
    </article>
...
삽입되어 렌더링된다. */
/* useLocation (Hook)
  : React Router을 통해 라우팅된 페이지에서 현재 URL(경로)와 관련된
...
const RouterHooks = () => {
  ...
  const nextPage = () => {
    /* 페이지 번호가 없는 상태라면 1로 지정하고, 값이 있다면 + 1 시켜준다. */
    let pageTemp = (pageNum===null || isNaN(pageNum)) ? 
      1 : parseInt(pageNum) + 1;
    // 최대 10페이지라는 가정. 최대 10페이지로 설정
    if(pageTemp === 11) {
      pageTemp = 10;
...
function App() {
  return (<>
    {/* 라우팅 처리가 필요없는 컴포넌트는 전체페이지에서 공통으로 렌더링되는
    Navigation을 주로 사용한다. */}
    <TopNavi />
    {/* 라우팅 처리가 필요한 컴포넌트는 아래와 같이 path, element라는
    속성(props)을 통해 경로와 렌더링할 컴포넌트를 지정한다. */}
    <Routes>
      <Route path="/" element={<Home />} />
      {/* 하위 경로가 필요한 경우에는 '중첩라우팅'을 사용한다. */}
      <Route path="/intro" element={<CommonLayout />}>
        {/* '/intro'로 요청이 들어오면 이 컴포넌트를 렌더링 (default Route) */}
        <Route index element={<LayoutIndex />} />
        {/* '/intro/router'와 같이 하위 경로로 요청이 들어오면
        RouterHooks 컴포넌트를 렌더링한다. */}
        <Route path="router" element={<RouterHooks />} />
      </Route>
      {/* 지정되지 않은 모든 경로에 대해서는 404처리 */}
