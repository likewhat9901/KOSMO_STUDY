...
훅은 import를 먼저 진행한 후 useXYZ()와 같은 패턴의 함수를 선언하여 사용하면 된다.
*/

/* 
useState()
: 리액트에서 상태값을 가지는 state의 값을 변경하거나 초기값을 부여할때
사용한다. 이 함수의 반환값은 배열인데 0번 요소는 state의 초기값을 저장하는
변수이고 1번 요소는 이 값을 변경하는 함수로 사용한다.
...
function App() {
  ...
  let contents = '';
  if(mode === 'front'){
    // mode값이 front이면 MyCont1 컴포넌트만 변수에 저장 후 랜더링한다.
    contents = <>
      {/* 각 컴포넌트에서는 myModeChange라는 프롭스를 전달하게 되는데,
      State로 정의한 mode의 값을 매개변수를 통해 변경하는 기능을 가지고 있다. */}
