...
function WriteForm(props){
  return (
    ...
    <form onSubmit={(e)=>{
      ...
      console.log(e.target.writer); // <input name="writer">
      console.log(e.target.writer.value); // 사용자가 입력한 텍스트
...
function App() {
  ...
  {/* 작성폼 컴포넌트를 추가하면서 프롭스를 통해 폼값을 받아 콘솔에 출력하는 함수 전달 */}
  <WriteForm writeAction={(wr, ti, con)=>{
    // 콘솔에서 확인
    console.log("Form값", wr, ti, con);
    // 3개의 값 모두 입력하면 완료로 변경
    if(wr!=='' && ti!=='' && con!==''){
      // 스테이트가 변경되면 리렌더링 된다.
      setMessage('폼값 검증 완료');
    }
    else{
      setMessage('폼값을 모두 입력하세요');
    }
  }}></WriteForm>
  {/* 스테이트가 변경되면 리렌더링이 되므로 아래 텍스트가 변경된다.
  즉, 폼값 검증이 완료되었을때 변경됨. */}
  <p>{message}</p>
  </>)
}
