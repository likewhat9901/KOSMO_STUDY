...
const GlobalTop = (props) => {
  ...
  useEffect(function() {
    ...
    fetch('./json/myData.json')
      ...
      .then((json)=>{
        ...
        이 값을 받은 후 데이터로 사용할 State를 변경한다. */
        console.log(json);
        setMyList(json);
      });
    return () =>{
      /* 컴포넌트가 언마운트될때 실행할 코드가 있다면 이 부분에 작성하면 됨.
  ...
  let listTag = [];
  /* State 변수의 크기만큼 반복한다. 최초 실행시에는 빈 배열이므로 for문은
  ...
  for (let i = 0; i < myList.length; i++) {  
    ...
    listTag.push(
      <li key={data.id}>
        ...
        dataset.id를 통해 읽어올 수 있다. 이 속성에 게시물의 일련번호인 num을
        설정하고 있다. */}
...
/* props로 전달된 객체의 값을 화면에 출력. 즉 내용을 출력. */
const ContentBody = (props) => {
  return (<>
    ...
    <ul>
      ...
      <li>cell : {props.myResult.description}</li>
...
function App() {  
  ...
  return (<>
    <h2>React - 내부서버통신</h2>
    {/* 클릭 시 내부에 저장된 dto.json 파일을 get방식으로 요청한 후
