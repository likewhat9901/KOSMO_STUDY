...
public class MainController {
	...
	//회원목록
	@GetMapping("/list.do")
	public String member2(Model model, MemberDTO dto) {
		/*
		 * DAO의 select 메서드 호출 후 반환되는 List<MemberDTO>를 영역에 저장한다.
		 * 검색 기능 추가: 목록에서 사용자가 입력한 검색필드, 검색어를 DTO를 통해
		 * 한번에 받으므로, 이것을 DAO로 전달해야 한다.
		 */
		
		model.addAttribute("memberList", dao.select(dto));
		return "list";
	}
