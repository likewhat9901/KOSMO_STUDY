...
public class MainController {
	...
	public String uploadProcess(HttpServletRequest req, Model model) {
		try {
			...
			//서버에 저장된 파일의 파일명을 UUID로 생성된 문자열로 변경.
	...
	public String multiUploadProcess(HttpServletRequest req, Model model) {
		try {
			...
			/*
			 * 파일명 저장을 위한 Map 생성. Key는 원본파일명, Value는 서버에
			 * 저장된 파일명으로 저장한다.
			 */
			...
			for(Part part : parts) {
				/*
				 * 여러 폼값 중 파일인 경우에만 업로드 처리를 위해 하위 문장을 실행.
				 * 파일이 아닌 경우에는 반복문의 처음으로 돌아간다.
				 */
